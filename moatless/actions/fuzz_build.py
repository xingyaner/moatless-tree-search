...
class FuzzBuild(Action):
    ...
    def execute(self, args: FuzzBuildArgs, file_context=None, workspace=None) -> Observation:
        ...
        try:
            ...
            if return_code == 0:
                check_command = [
                    "python3", helper_path, "check_build",
                    self.project_name,
                    "--sanitizer", self.sanitizer,
                    "--engine", self.engine,
                    "--architecture", self.architecture,
                ...
                check_log_content = []
                for line in check_process.stdout:
                    print(line, end='', flush=True)
                    check_log_content.append(line)

                check_process.wait()
                check_return_code = check_process.returncode
                check_output = "".join(check_log_content)
                final_log = final_log + "\n--- [check_build] ---\n" + check_output
            ...
            # 3. 判定逻辑
            # 仅以 check_build 通过作为成功判定标准
            is_success = (return_code == 0 and check_return_code == 0)
